...
const inputBusqueda = document.querySelector("#busqueda-tareas");
const btnModo = document.querySelector("#btn-modo");

let filtroActual = "todas";
let terminoBusqueda = "";
let temaActual = localStorage.getItem("taskflow-tema") || "claro";
...
async function renderizarTareas() {
  let tareas = await obtenerTareas(filtroActual);

  if (terminoBusqueda) {
    const termino = terminoBusqueda.toLowerCase();
    tareas = tareas.filter(t => t.titulo.toLowerCase().includes(termino));
  }
...
function aplicarTema(tema) {
  ...
  btnModo.textContent = tema === "oscuro" ? "☀️" : "🌙";
  btnModo.title = tema === "oscuro" ? "Cambiar a modo claro" : "Cambiar a modo oscuro";
}

btnModo.addEventListener("click", () => {
  temaActual = temaActual === "oscuro" ? "claro" : "oscuro";
  localStorage.setItem("taskflow-tema", temaActual);
